...
* ```--pretty``` 옵션은 히스토리 내용을 보여줄 때 기본 형식 이외에 여러 가지 중에 하나를 선택할 수 있다.
    ...
    * ```format```은 나만의 포맷으로 결과를 출력하고 싶을 때 사용한다. ex)  `git log --format="%H %an %ar" |  grep "Merge into '"'"'master'"'"' "` 명령어를 통해 Merge into master로 머지커밋의 SHA-1을 알 수 있다. -> 리눅스에서 grep을 할때 grep할 문자열 안에 ' 기호가 있는경우 '"'"'으로 치환하여 찾아야 한다.
